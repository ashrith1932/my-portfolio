...
const capabilities = [
  { icon: Code2, title: 'Algorithmic foundation', text: 'I use C, Java, data structures and contest practice to break ambiguous problems into precise, testable steps.', context: 'The foundation underneath every system I build.' },
  { icon: BrainCircuit, title: 'Evidence-first AI', text: 'I work with RAG, semantic search, BM25 and local LLM inference when a system needs traceable reasoning rather than a black-box answer.', context: 'Most visible in Source-Bias Analyzer.' },
  { icon: Layers3, title: 'End-to-end product work', text: 'I connect interfaces to APIs, authentication, data models and real workflows so a project is useful beyond the demo screen.', context: 'Most visible in CitiSolve and Nerdding.' },
  { icon: Database, title: 'Systems and data thinking', text: 'I care about how information moves: collection, retrieval, validation, storage, permissions and the final interface that makes it understandable.', context: 'The thread connecting AI and full-stack work.' },
];
...
function App() {
  ...
  return <div className={`site ${lightMode ? 'theme-light' : ''}`}>
    ...
    <main id="top">
      ...
      <section className="section shell capabilities-section"><Reveal><div className="section-head"><div><span className="kicker">CAPABILITIES / 03</span><h2>What I bring to a team.</h2></div><span className="section-index">CONNECTED SKILLS</span></div><p className="capabilities-lede">My capabilities are not separate labels on a stack. They work together: I start with a problem, choose the right level of abstraction, build the system behind it and keep the result understandable for the person using it.</p></Reveal><div className="cap-grid">{capabilities.map((capability, index) => { const Icon = capability.icon; return <Reveal key={capability.title} delay={index * 0.06}><article className="cap"><div className="cap-icon"><Icon size={19} /></div><span>0{index + 1}</span><h3>{capability.title}</h3><p>{capability.text}</p><small>{capability.context}</small></article></Reveal>; })}</div></section>
      ...
      <section className="section shell proof-section"><Reveal><div className="section-head"><div><span className="kicker">PRACTICE / 05</span><h2>Achievements with context.</h2></div><span className="section-index">COMPETITIVE PROGRAMMING</span></div><p className="proof-lede">Competitive programming is where I practise the habits that show up in my projects: reading constraints carefully, finding the right abstraction, and staying precise when the solution has to work under pressure.</p></Reveal><div className="proof-grid"><a href={LEETCODE} target="_blank" rel="noreferrer" className="proof-card"><Trophy /><div><b>LeetCode</b><strong>1811</strong><span>Max rating. Global rank 1944 in Biweekly Contest 181 — evidence of consistency across timed problem solving.</span></div><ArrowUpRight /></a><a href={CODECHEF} target="_blank" rel="noreferrer" className="proof-card"><Trophy /><div><b>CodeChef</b><strong>1459</strong><span>Max rating. Global rank 1684 in Starters 213 — translating a fresh problem statement into working code quickly.</span></div><ArrowUpRight /></a><a href={CODEFORCES} target="_blank" rel="noreferrer" className="proof-card"><Code2 /><div><b>Codeforces</b><strong>1166</strong><span>Max rating. A steady practice loop for algorithms, implementation speed and learning from failed approaches.</span></div><ArrowUpRight /></a></div><div className="toolbelt"><span>TOOLBELT</span>{['C', 'Java', 'React', 'Node.js', 'Express', 'MongoDB', 'Python', 'FAISS', 'Ollama'].map((item) => <b key={item}><TechMark label={item} />{item}</b>)}</div></section>
